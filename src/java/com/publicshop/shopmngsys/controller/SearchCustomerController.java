...
public class SearchCustomerController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            System.out.println("c1111111111");
            CustomerService customerService = new ServiceFactory().getCustomerService();
            System.out.println("c2222222222");
            CustomerDto customer = (CustomerDto) request.getAttribute("customer");

            if (null == customer) {
                customer = new CustomerDto();
            }
            CustomerDto customerDto = customerService.searchCustomer(name);
            if (null == customerDto) {
                out.print("<script language='javaScript'>alert('Customer not Found)</script>");
            } else {
